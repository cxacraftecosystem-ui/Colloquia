...
class TokenStore(context: Context) {
    private val preferences = context.getSharedPreferences("transcriptai_auth", Context.MODE_PRIVATE)
    ...
    fun setUser(user: UserDto?) {
        preferences.edit().apply {
            if (user == null) remove(KEY_USER)
            else putString(KEY_USER, json.encodeToString(UserDto.serializer(), user))
        }.apply()
    }
