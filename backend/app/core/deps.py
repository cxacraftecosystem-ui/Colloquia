...
def assert_owns(record: Any, user: Any, owner_field: str = "userId") -> None:
    """Every recording/folder/tag belongs to exactly one user; admins may reach anything."""
    if is_admin(user):
        return
    if get_value(record, owner_field) != get_value(user, "id"):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="You can only access your own records"
        )
